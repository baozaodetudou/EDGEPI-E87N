...
Debian package, and reproducible build/test evidence. Firmware and Display use
independent manual workflows, tags and Releases. A Firmware Release publishes
only the firmware TAR; a Display Release publishes only the display deb.
...
| Complete firmware build | clean compiler exit, image, firmware TAR, embedded same-source display baseline, hashes, source/config provenance |
...
| Display package lifecycle | independent package build and install/upgrade/remove checks; hardware display test remains pending |
| Firmware release gate | publication requires matching firmware simulation evidence; manual-only workflow; one TAR attachment |
| Display release gate | publication requires package lifecycle evidence; manual-only workflow; one deb attachment |
