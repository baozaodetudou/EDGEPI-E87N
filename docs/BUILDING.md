...
当前屏幕/风扇候选为 **Debian 13.6 Trixie + Linux 6.18.51-current-filogic**。2026-09-13 的完整镜像构建、真实镜像只读检查和导出校验已完成，证据见[候选记录](candidate-display-fan-20260913.md)。尚未在 E87N 上启动或测试。本文记录仓库现有构建入口；构建后的检查见 [TESTING.md](TESTING.md)。

下列命令均从仓库根目录执行。产物路径属于本地构建目录，不表示仓库已经提供 GitHub 镜像下载或自动构建服务。
...
| 目标发行版 | `RELEASE=trixie`；已交付候选实测为 Debian 13.6 |
| 内核配置 | `userpatches/config/kernel/linux-edgepi-e87n-lts.config`，另有板卡 hook 调整最终配置 |
| 当前补丁目录 | `userpatches/kernel/edgepi-e87n-6.18/`，共 13 个补丁，包括 `900-fb-nv3007-e87n.patch` |
| 用户空间与固件 | `board-support/`、`userpatches/customize-image.sh`、`firmware/`；PHY 固件安装前检查 SHA-256 与大小 |

这些 pin 固定框架和内核源码，不构成逐字节可复现的整个系统快照。`customize-image.sh` 会从选定发行版的签名软件源更新软件包，安装 `python3`、`python3-pil` 和 `fonts-dejavu-core`。它核对 Debian 13 / Trixie 身份，但不锁定所有包的版本，也不强制未来构建仍显示 13.6。Docker 默认镜像标签同样会变化。
...
准备完成后，启动器把原框架 `userpatches/` 移入新建的 `userpatches-backup.*` 目录，再复制本仓库的 overlay，并加入 `firmware/` 和 `board-support/`。后续修改应落在本仓库的输入文件中；直接修改框架内复制的 overlay 不会自动回传。
...
3. 将本次仓库输入手动复制到 VM 原生文件系统的工作目录，默认 `/srv/e87n`。至少保留构建脚本、`scripts/`、`patches/`、`userpatches/`、`firmware/` 和 `board-support/` 的相对关系；若要运行测试，同时复制 `tests/` 和 `README.md`。
