...
目标 chroot 中调用该脚本，再通过 APT 安装产物。本改动提供这一接口，接入由镜像
构建部分负责；本文不代表新镜像已构建。
...
./scripts/build-display-deb.sh --output-dir ./output/debs --version 1.0.1-1
```

产物严格命名为 `e87n-display_VERSION_all.deb`，例如
`output/debs/e87n-display_1.0.1-1_all.deb`。标准输出包含构建进度，调用方应按版本
...
所有权。macOS 缺少 dpkg 时会明确报错，可在已有 Linux VM 中构建。
...
原样复制。依赖为 `python3`、`python3-pil`、
`fonts-dejavu-core`，以及服务维护工具所在的 `init-system-helpers (>= 1.56)`。
...
```sh
sudo apt-get install ./e87n-display_1.0.1-1_all.deb
...
sudo apt-get -y -o Dpkg::Options::=--force-confdef \
  -o Dpkg::Options::=--force-confold install ./e87n-display_1.0.1-1_all.deb
