...
class SystemAudit(unittest.TestCase):
    def setUp(self):
        ...
        # Real Debian 13 openssh-server 1:10.0p1-7+deb13u4 unit, also present
        # in Actions 34735890073. Do not invent an absolute-path-only fixture.
        self.put("/usr/lib/systemd/system/sshd-keygen.service",
                 (REPO / "tests/fixtures/systemd/sshd-keygen.service").read_bytes())
        self.put("/usr/bin/ssh-keygen", b"fixture: never executed")
        self.file("/usr/bin/ssh-keygen").chmod(0o755)
    ...
    def test_keygen_command_whitespace(self):
        self.put("/usr/lib/systemd/system/sshd-keygen.service",
                 b"[Service]\n  ExecStart = ssh-keygen\t-A  \n")
        self.check()

    def test_invalid_keygen_commands(self):
        for contents in (
            "[Service]\n", "[Service]\n# ExecStart=ssh-keygen -A\n",
            "[Unit]\nExecStart=ssh-keygen -A\n[Service]\n",
            "[Service]\nExecStart=ssh-keygen\n", "[Service]\nExecStart=/tmp/ssh-keygen -A\n",
            "[Service]\nExecStart=-ssh-keygen -A\n",
            "[Service]\nExecStart=/bin/sh -c 'ssh-keygen -A'\n",
            "[Service]\nExecStart=ssh-keygen -A\nExecStart=\n",
            "[Service]\nExecStart=ssh-keygen -A\nExecStart=/bin/true\n",
            "[Service]\nExecStart=ssh-keygen -A ; /bin/true\n",
            "[Service]\nExecStart=ssh-keygen -A --unexpected\n",
            "[Service]\nExecStart=ssh-keygen -A\\\n --unexpected\n",
        ):
            with self.subTest(contents=contents):
                self.put("/usr/lib/systemd/system/sshd-keygen.service", contents.encode())
                with self.assertRaisesRegex(ValueError, "unexpected ExecStart"):
                    self.check()

    def test_invalid_keygen_binary(self):
        binary = self.file("/usr/bin/ssh-keygen")
        for mode, uid in ((0o644, 0), (0o777, 0), (0o755, 1000)):
            with self.subTest(mode=mode, uid=uid):
                binary.chmod(mode)
                os.chown(binary, uid, 0)
                with self.assertRaisesRegex(ValueError, "root-owned, non-writable executable"):
                    self.check()

    def test_missing_keygen_binary(self):
        self.file("/usr/bin/ssh-keygen").unlink()
        with self.assertRaises(OSError):
            self.check()

    def test_debian_merged_usr_and_service_aliases(self):
        self.file("/lib").symlink_to("usr/lib")
        self.file("/etc/systemd/system/sshd.service").symlink_to("/usr/lib/systemd/system/ssh.service")
        link = self.file("/etc/systemd/system/multi-user.target.wants/ssh.service")
        link.unlink()
        link.symlink_to("/lib/systemd/system/ssh.service")
        link = self.file("/etc/systemd/system/multi-user.target.wants/systemd-timesyncd.service")
        target = self.file("/etc/systemd/system/sysinit.target.wants/systemd-timesyncd.service")
        target.parent.mkdir(parents=True)
        link.rename(target)
        self.check()

    def test_debian_locale_symlink(self):
        self.put("/etc/locale.conf", b'# Generated by update-locale\nLANGUAGE=zh_CN:zh\nLANG=zh_CN.UTF-8\n')
        self.file("/etc/default/locale").unlink()
        self.file("/etc/default/locale").symlink_to("../locale.conf")
        self.check()
